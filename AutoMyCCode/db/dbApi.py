...
def createTable(cursor, connect, tablename):
    try:
      ...
      `content` TEXT  COMMENT  'content for show',
      PRIMARY KEY (`id`),
...
def insert_content(databasename,tablename, host, usr,pwd,port,charset,data):
    ##################################### main functions#######################
    connect,result = initDatabase(host, usr, pwd,port,charset)
    if "error" in result:
        return (0xFFFF)
    # 获取游标
    cursor = connect.cursor()
...
def test_insert_contet(data=('Qt si good', 'Qt是什么内容呢')):
    insert_content('alldb', 'abc', 'localhost', 'root', '123456', 3306, 'utf8',
                   data)
...
def default_search_contetName(databasename='alldb',
                                      tablename='abc',
                                      host='localhost',
                                      usr='root',
                                      pwd='123456',
    ...
    # insertTable(cursor, connect, tablename)
    # modifyTable(cursor, connect, tablename)
    things = searchTale(cursor, connect, tablename,data)
    # deleteTable(cursor, connect, tablename)
    # atomCommit(cursor, connect, tablename)
    closeDatabase(cursor, connect)
    return  things
...
if __name__ == '__main__':
    # one_pymysql_test('python10','a6', 'localhost', 'root', '123456',3306,'utf8')
